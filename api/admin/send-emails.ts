...
/** POST /api/admin/send-emails { codigos: string[], force?: boolean } (admin). */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { codigos, force } = (req.body ?? {}) as { codigos?: string[]; force?: boolean };
  if (!Array.isArray(codigos) || codigos.length === 0) {
    return res.status(400).json({ error: "Selecione ao menos um pedido." });
  }
  const result = await sendOrderEmails(codigos.map(String), !!force);
